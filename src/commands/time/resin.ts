...
export default class Resin extends Command {
    ...
    async run(message: Message, args: string[]): Promise<Message | Message[]> {
        ...
        if (isNaN(currentResin) || currentResin < 0 || currentResin > max_resin)
            return message.channel.send(`Invalid current resin should be a number between 0 and ${max_resin}`)
        ...
        if (split.length < 2)
            ...
            return message.channel.send("Invalid time left until next resin should be in mm:ss format")

        const [mm, ss] = split

        if (isNaN(mm) || mm < 0 || mm > minutes_per_resin)
            return message.channel.send(`Invalid minutes until next resin should be a number between 0 and ${minutes_per_resin} minutes`)
        if (isNaN(ss) || ss < 0 || ss > 59 || (mm == minutes_per_resin && ss != 0))
            return message.channel.send("Invalid time until next resin, seconds should be a number between 0 and 59 seconds")
